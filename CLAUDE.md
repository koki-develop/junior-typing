...
- `src/pages/` — one component per route. Each page owns its own shell (`<main>` wrapper, layout) and composes `features/` inside. Adding a page never changes shared layout unless a common shell actually emerges.
  - `TopPage.tsx` — the `/` route. A selector page (intentionally light on chrome — no marketing copy / how-to bullets) laid out as: `<header>` (小さなロゴマーク「あ」+ プロダクト名) → `<main>` (カテゴリごとの `<section>`)。 Grouping is driven by `CATEGORIES` — the display order and the color-dot color of each category come from `CATEGORIES`, not from `questionSets`' declaration order. `questionSets` is filtered per category via `set.category === category.id`, and a category with zero matching sets is skipped entirely, so appending a new set (or a new category) to the data flows through with no UI change. Each card is a `<Link to="/play/$setId">` showing `set.title` + a sample-words preview via `previewWords(set)` (先頭 8 件を `・` で連結、はみ出しは CSS `truncate` で 1 行省略）。 The hi-score / 未挑戦 表示エリアは今は無い（機能未実装のため区切り線ごと省略。実装するときはカード内の下段にこの領域だけ足す想定）。 ホバーは `border-accent` + `shadow-md` のみで `transition` は乗せない（デザインの明示要件: アニメーション無し）。
