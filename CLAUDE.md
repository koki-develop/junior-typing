...
- **New question set**: append a new `QuestionSet` entry to `questionSets` in `src/domain/questions/questions.ts` with a unique `id` (URL-safe; goes into `/play/$setId`) and a display `title`. No routing / UI changes needed — `TopPage` iterates `questionSets` and `router.ts`'s `beforeLoad` uses `findQuestionSet(id)` to resolve it. `questions.test.ts` re-runs against every set automatically.
