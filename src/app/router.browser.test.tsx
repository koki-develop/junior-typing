...
test("`/play/animals` は PlayPage を描画する（IdleMessage が表示される）", async () => {
  const router = createTestRouter("/play/animals");
  const screen = await render(<RouterProvider router={router} />);
...
test("setId が変わると PlayPage が丸ごとマウントし直され、直前のプレイ状態を引きずらない", async () => {
  ...
  await runInAct(() => router.navigate({ to: "/play/$setId", params: { setId: "foods" } }));
