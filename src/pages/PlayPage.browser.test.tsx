...
// テストは常に /play/sample で PlayPage をマウントする。PlayPage は useParams({ from: "/play/$setId" })
// を使うのでルータコンテキスト必須。プロダクションと同じ routeTree を memory history で回す。
function renderPlayPage() {
  const router = createRouter({
    routeTree,
    history: createMemoryHistory({ initialEntries: ["/play/sample"] }),
  });
  return render(<RouterProvider router={router} />);
}

const sampleSet = findQuestionSet("sample");
if (!sampleSet) throw new Error("test fixture missing: questionSets に 'sample' がない");
const questions = sampleSet.questions;
...
test("全問題を最後まで打ち終えると ResultScreen（スコアと もういちど ボタン）が表示される", async () => {
  const screen = await renderPlayPage();
...
test("結果画面から Space キーで idle に戻り、再度プレイを開始できる", async () => {
  ...
  // idle に戻ればスタート案内が再度出てくる。
  await expect.element(screen.getByText("スタート")).toBeVisible();

  // そのままスペースキーでもう一度ゲームを始められる。
  await pressKey(" ");
  await advanceTimers(COUNTDOWN_STEP_MS * 3);
  await expect.element(screen.getByText(questions[0].text).first()).toBeVisible();
});
