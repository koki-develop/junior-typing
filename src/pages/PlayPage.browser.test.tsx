...
// done フェーズでは ResultScreen 側にも同じアクセシブルネーム「もどる」のリンクが
// 表示される（PlayPage ヘッダーと行き先(/)が同じ操作に別々の呼び名をつけないための
// 意図的な文言統一。詳細は ResultScreen.tsx のコメント参照）。そのため『もどる』という
// 名前だけでは要素が一意に定まらず、ヘッダーリンクを指したいテストは <header> の
// banner ランドマークでスコープして区別する。
function headerBackLink(screen: Awaited<ReturnType<typeof renderPlayPage>>) {
  return screen.getByRole("banner").getByRole("link", { name: "もどる" });
}

test("ヘッダーの『もどる』リンクは idle / countdown / playing / done のどのフェーズでも常に表示される", async () => {
  const screen = await renderPlayPage();

  // idle
  await expect.element(headerBackLink(screen)).toBeVisible();
...
test("ヘッダーの『もどる』リンクを押すとトップページに戻る", async () => {
  const screen = await renderPlayPage();

  await headerBackLink(screen).click();

  // TopPage のヘッダ見出し。/play からトップへ確かに遷移したことの目印にする。
  await expect.element(screen.getByRole("heading", { name: "ジュニアタイピング" })).toBeVisible();
});

test("ヘッダーの『もどる』リンクには press=press の cuelume 属性が付く", async () => {
  const screen = await renderPlayPage();

  await expect.element(headerBackLink(screen)).toHaveAttribute("data-cuelume-press", "press");
});

test("questionCount 件を打ち終えると ResultScreen にも『もどる』リンクが表示され、押すとトップページに戻る", async () => {
  const screen = await renderPlayPage();
  ...
  // done フェーズではヘッダーと ResultScreen の 2 箇所に「もどる」リンクが存在する。
  // ResultScreen 側は「もういちど」ボタンの直後（DOM 順で 2 番目）に描画されるので .last() で拾う。
  const resultBackLink = screen.getByRole("link", { name: "もどる" }).last();
  await expect.element(resultBackLink).toBeVisible();
  await expect.element(resultBackLink).toHaveAttribute("data-cuelume-press", "press");

  await resultBackLink.click();
  await expect.element(screen.getByRole("heading", { name: "ジュニアタイピング" })).toBeVisible();
});
