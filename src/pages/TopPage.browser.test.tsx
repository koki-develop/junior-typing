...
test("カードにサンプル語プレビューが表示される", async () => {
  ...
  // 各セットの先頭の問題テキストが、そのセット自身のカード内に載っていることを確認する
  // （プレビュー実装の内部は追わず「中身が伝わる語が出ている」ことだけ検証）。
  // 他セットのプレビュー文字列に部分文字列として現れることがある（例: 「船」が「飛行船」に
  // マッチする）ため、set.title のカードにスコープして検索し、他カードとの衝突を避ける。
  for (const set of questionSets) {
    const firstText = set.questions[0].text;
    const card = screen.getByRole("link", { name: new RegExp(set.title) });
    await expect.element(card.getByText(firstText, { exact: false })).toBeVisible();
  }
});
