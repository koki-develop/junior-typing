...
test("スペースキーで countdown が始まり bloom 音が鳴る", async () => {
  ...
  expect(hook.result.current).toMatchObject({ phase: "countdown", count: 3 });
...
test("countdown が3段進むと playing に入り最初の問題を読み込む", async () => {
  ...
  if (hook.result.current.phase === "playing") {
    expect(hook.result.current.question).toEqual(twoQuestions[0]);
  }
...
test("正解の最終打鍵で cleared になり、page と success が両方鳴る", async () => {
  ...
  expect(hook.result.current).toMatchObject({ phase: "playing", cleared: true });
...
test("CLEAR_DELAY_MS 経過後に次の問題へ進む", async () => {
  ...
  expect(hook.result.current.phase).toBe("playing");
  if (hook.result.current.phase === "playing") {
    expect(hook.result.current.questionIndex).toBe(1);
    expect(hook.result.current.question).toEqual(twoQuestions[1]);
    expect(hook.result.current.cleared).toBe(false);
  }
});

test("2問とも打ち終えると done になる", async () => {
  const hook = await renderHook(() => useTypingGame(twoQuestions));
  await pressKey(" ");
  await advanceTimers(COUNTDOWN_STEP_MS * 3);
...
// countdown 中に questions が差し替わっても、schedule された tick は sendRef 経由で
// 常に「最新レンダーの send（＝最新の questions を閉じ込めた send）」を呼ぶ
// （useTypingGame.ts の sendRef, 53-56行目）。ここが壊れると countdown 完了時に
// 古い questions の問題が読み込まれてしまう。
test("countdown 中に questions が差し替わると、新しい questions の先頭問題で playing に入る", async () => {
  const questionsA: Question[] = [{ text: "あ", kana: "あ" }];
  const questionsB: Question[] = [{ text: "い", kana: "い" }];
  ...
  expect(hook.result.current.phase).toBe("playing");
  if (hook.result.current.phase === "playing") {
    expect(hook.result.current.question).toEqual(questionsB[0]);
  }
});
