...
// cleared による見出し色・花丸（HanamaruStamp）の出し分けに絞る。
...
test("cleared=false のとき見出しは text-ink で、花丸（svg）は描画されない", async () => {
  const screen = await render(
...
test("cleared=true のとき見出しは text-emerald-500 になり、花丸（svg）が重なる", async () => {
  const screen = await render(
...
test("花丸（svg）はクリックを奪わず装飾専用（pointer-events-none + aria-hidden）", async () => {
  const screen = await render(
    <QuestionDisplay kana="かき" text="柿" typed="kaki" next="" rest="" cleared={true} />,
  );
